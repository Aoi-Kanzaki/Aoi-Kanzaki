...
class Leveling(commands.Cog):
    ...
    async def cog_load(self):
        async with aiosqlite.connect("./data/level.db") as db:
            await db.execute(
                "CREATE TABLE IF NOT EXISTS levels (level INTEGER, xp INTEGER, xpcap INTEGER, user INTEGER, guild INTEGER)"
            )
            await db.execute(
    ...
    async def on_message(self, message):
        ...
        async with aiosqlite.connect("./data/level.db") as db:
            ...
            if xp >= xpcap[0]:
                ...
                await db.execute(
                    "UPDATE levels SET xpcap = ? WHERE user = ? AND guild = ?",
                    (
                        xpcap,
                        author.id,
                        guild.id,
                ...
                if role:
                    ...
                    try:
                        ...
                        await message.channel.send(
                            f"{author.mention} has leveled up to **{level}**! Enjoy your new **{role.name}** role!"
                        )
                    except discord.HTTPException:
                        await message.channel.send(
                            f"{author.mention} has leveled up to **{level}**! [WAS NOT ABLE TO GIVE LVL UP ROLE]"
                        )
                else:
                    await message.channel.send(
                        f"{author.mention} has leveled up to **{level}**!"
                    )
            await db.commit()
