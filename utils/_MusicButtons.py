...
class queue_msg_buttons(discord.ui.View):
    ...
    async def prev_page(
        ...
        draw_queue = await self.bot.get_cog("Music").draw_queue(
            self.guild_id, self.page
        )
        ...
        e.set_footer(
            text=f"Page {self.page}/{math.ceil(len(self.player.queue) / 10)} | {len(self.player.queue)} tracks"
        )
        e.set_thumbnail(
...
class np_msg_buttons(discord.ui.View):
    ...
    async def queue(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not self.player.queue:
            return await interaction.response.send_message(
                content="Nothing playing.", ephemeral=True
            )
        if self.player.current.stream:
            dur = "LIVE"
        else:
        ...
        e.add_field(name="Up Next:", value=f"{draw_queue}", inline=False)
        e.set_footer(
            text=f"Page 1/{math.ceil(len(self.player.queue) / 10)} | {len(self.player.queue)} tracks"
        )
        e.set_thumbnail(
            url=f"https://img.youtube.com/vi/{self.player.current.identifier}/hqdefault.jpg"
        )
        if len(self.player.queue) > 10:
            return await interaction.response.edit_message(
                embed=e, view=queue_msg_buttons(self.bot, self.guild_id, 1)
            )
        else:
    ...
    async def pause(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.player.is_playing:
            ...
            e.description += (
                f"{draw_time} `[{format_time(self.player.position)}/{dur}]`\n"
            )
    ...
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.player.is_playing:
            ...
            return await interaction.response.send_message(
                content="<:tickYes:697759553626046546> Skipped.", ephemeral=True
            )
        else:
            return await interaction.response.send_message(
                content="Nothing playing.", ephemeral=True
    ...
    async def stop(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.player.is_playing:
            ...
            await interaction.response.send_message(
                content="⏹️ Stopped music and cleared queue.", ephemeral=True
            )
            return await interaction.message.delete()
        else:
            return await interaction.response.send_message(
...
class event_hook_buttons(discord.ui.View):
    ...
    async def previous(
        ...
        if self.player.fetch("prev_song") is None:
            return await interaction.response.send_message(
                content="No previous track.", ephemeral=True
            )
        last_track = await self.player.node.get_tracks(self.player.fetch("prev_song"))
        if not last_track:
            return interaction.response.edit_message(
                content="Seems there was an issue in getting the last track and nothing was found.",
                embed=None,
                view=None,
            )
        self.player.add(
            requester=self.player.fetch("prev_requester"), track=last_track["tracks"][0]
        )
        ...
        async with aiosqlite.connect("./data/music.db") as db:
            ...
            if data[2] == self.player.fetch("channel"):
                return await interaction.response.send_message(
                    content="<:tickYes:697759553626046546> Replaying Track.",
                    ephemeral=True,
                )
            else:
                embed = discord.Embed(
                ...
                if self.player.paused == False:
                    ...
                    if self.player.queue:
                        ...
                        for i, track in enumerate(
                            self.player.queue[(1 - 1) * 5 : (1 - 1) * 5 + 5],
                            start=(1 - 1) * 5,
                        ):
                    ...
                    if self.player.queue:
                        queue_list = ""
                        for i, track in enumerate(
                            self.player.queue[(1 - 1) * 10 : (1 - 1) * 10 + 10],
                            start=(1 - 1) * 10,
                        ):
                            queue_list += "`{}.` {}\n".format(i + 1, track.title)
                    else:
