...
class Levels(commands.Cog):
    ...
    async def level(self, ctx, member: discord.Member = None):
        ...
        async with aiosqlite.connect("./data/level.db") as db:
            levelsys = await db.execute(
                "SELECT levelsys FROM levelSettings WHERE guild = ?", (ctx.guild.id,)
            )
            ...
            xpcap = await db.execute(
                "SELECT xpcap FROM levels WHERE user = ? AND guild = ?",
                (
                    member.id,
                    ctx.guild.id,
                ),
            ...
            if not xp or not level:
                await db.execute(
                    "INSERT INTO levels (level, xp, xpcap, user, guild) VALUES (?, ?, ?, ?, ?)",
                    (
                        0,
                        0,
                        1000,
    ...
    async def rewards(self, ctx):
        ...
        async with aiosqlite.connect("./data/level.db") as db:
            ...
            roles = await db.execute(
                "SELECT * FROM levelSettings WHERE guild = ?", (ctx.guild.id,)
            )
            roleLevels = await roles.fetchall()
            if not roleLevels:
                return await ctx.send("No role levels have been setup for this guild!")
            e = discord.Embed(
                title="Role Levels:", description="Role levels for this server."
            )
            for role in roleLevels:
                e.add_field(
                    name=f"Level {role[2]}",
                    value=f"{ctx.guild.get_role(role[1])}",
                    inline=False,
                )
    ...
    async def setrole(self, ctx, level: int, *, role: discord.Role):
        ...
        async with aiosqlite.connect("./data/level.db") as db:
            ...
            if roleTF or levelTF:
                return await ctx.send(
                    "A role or level setting for that value already exists."
                )
            await db.execute(
                "INSERT INTO levelSettings VALUES (?, ?, ?, ?)",
                (
