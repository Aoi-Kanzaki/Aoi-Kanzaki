...
class Playlists(commands.Cog):
    ...
    async def show(self, ctx, *, playlist: str = None):
        ...
        if not data or data == settings:
            return await ctx.send(
                "<:tickNo:697759586538749982> You don't have any playlists."
            )
        ...
        if playlist is not None:
            ...
            if playlist not in keys:
                return await ctx.send(
                    "<:tickNo:697759586538749982> Playlist not found with that name.",
                    delete_after=5,
                )
            e.set_author(
                name=f"More info on playlist {playlist}:", icon_url=ctx.author.avatar
            )
            ...
            if number > 0:
                if len(data[playlist]) < 10:
                    e.add_field(
                        name="Playlist links:",
                        value="\n".join(link for link in data[playlist]),
                        inline=False,
                    )
                elif len(data[playlist]) == 10:
                    e.add_field(
                        name="Playlist links:",
                        value=f"I cannot show all the links.",
                        inline=False,
                    )
                elif len(data[playlist]) > 10:
                    e.add_field(
                        name="Playlist links:",
                        value=f"I can only show the first 10 links.",
                        inline=False,
                    )
    ...
    async def add(self, ctx, name: str, *, link: str = None):
        ...
        if name is None:
            return await ctx.send(
                "<:tickNo:697759586538749982> Playlist name cannot be empty.",
                delete_after=5,
            )
        if not data:
            await self.bot.db.playlists.insert_one({"_id": ctx.author.id})
        await self.bot.db.playlists.update_one(
            {"_id": ctx.author.id}, {"$set": {name: []}}
        )
        if link is not None:
            await self.bot.db.playlists.update_one(
                {"_id": ctx.author.id}, {"$push": {name: link}}
            )
