...
class Mod(commands.Cog):
    ...
    async def cog_load(self):
        async with aiosqlite.connect("./data/prefixes.db") as db:
            await db.execute("CREATE TABLE IF NOT EXISTS prefixs (prefix STRING, guild INTEGER)")
            await db.commit()
        print(color("Prefixes loaded!", fore=self.bot.colors['blue']))
    ...
    async def prefix(self, ctx, prefix):
        ...
        async with aiosqlite.connect("./data/prefixes.db") as db:
            getData = await db.execute("SELECT * FROM prefixs WHERE guild = ?", (ctx.guild.id,))
            data = await getData.fetchone()
            if data is None:
                await db.execute("INSERT INTO prefixs VALUES (?, ?)", (prefix, ctx.guild.id))
                await ctx.send(f"<:tickYes:697759553626046546> Prefix set to `{prefix}`")
            else:
                await db.execute("UPDATE prefixs SET prefix = ? WHERE guild = ?", (prefix, ctx.guild.id))
                await ctx.send(f"<:tickYes:697759553626046546> Prefix set to `{prefix}`")
            await db.commit()
    ...
    async def kick(self, ctx, member: discord.Member, *, reason: str=None):
        ...
        try:
            ...
            await ctx.send(f"<:tickYes:697759553626046546> {member.mention} has been kicked from the server.", delete_after=5)
    ...
    async def ban(self, ctx, member: discord.Member, *, reason: str=None):
        ...
        try:
            ...
            await ctx.send(f"<:tickYes:697759553626046546> {member.mention} has been banned from the server.", delete_after=5)
        except discord.Forbidden:
