...
class Music(commands.Cog):
    ...
    async def cog_load(self):
        async with aiosqlite.connect("./data/music.db") as db:
            await db.execute(
                "CREATE TABLE IF NOT EXISTS musicSettings (musicMessage INTEGER, musicToggle INTEGER, musicChannel INTEGER, musicRunning INTEGER, guild INTEGER)"
            )
            await db.commit()
    ...
    async def track_hook(self, event):
        ...
        if isinstance(event, lavalink.events.NodeConnectedEvent):
            print(
                color(
                    f"Lavalink succesfully connected to node:",
                    fore=self.bot.colors["cyan"],
                ),
                color(f"{event.node.name}", fore=self.bot.colors["purple"]),
            )
        ...
        if isinstance(event, lavalink.events.TrackEndEvent):
            if event.player.fetch("npmsg") != None:
                try:
                    ...
                    ).fetch_message(event.player.fetch("npmsg"))
                    await msg.delete()
                ...
                if data[2] == event.player.fetch("channel"):
                    if event.player.queue:
                        ...
                        for i, track in enumerate(
                            event.player.queue[(1 - 1) * 5 : (1 - 1) * 5 + 5],
                            start=(1 - 1) * 5,
                        ):
                    ...
                    e.set_image(
                        url=f"https://img.youtube.com/vi/{event.player.current.identifier}/hqdefault.jpg"
                    )
                    ...
                    await msg.edit(
                        embed=e,
                        view=event_hook_buttons(self.bot, int(event.player.guild_id)),
                    )
                else:
                    if event.player.fetch("npmsg") != None:
                        try:
                    ...
                    if self.bot.get_channel(event.player.fetch("channel")):
                        try:
                            ...
                            message = await self.bot.get_channel(
                                ...
                                embed=e,
                                view=event_hook_buttons(
                                    self.bot, int(event.player.guild_id)
                                ),
                            )
    ...
    async def setup(self, ctx):
        ...
        async with aiosqlite.connect("./data/music.db") as db:
            getData = await db.execute(
                "SELECT musicToggle, musicChannel FROM musicSettings WHERE guild = ?",
                (ctx.guild.id,),
            )
            data = await getData.fetchone()
            if data is None:
                if "fresh-music" in existing_channels:
                    return await ctx.send(
                        "<:tickNo:697759586538749982> Please delete the `fresh-music` channel first.",
                        delete_after=5,
                    )
                ...
                await db.execute(
                    "INSERT INTO musicSettings (musicMessage, musicToggle, musicChannel, musicRunning, guild) VALUES (?, ?, ?, ?, ?)",
                    (
                        msgid,
                        1,
                        created.id,
                ...
                await ctx.send(
                    "<:tickYes:697759553626046546> Music channel setup complete. You can now move the channel to wherever you want."
                )
            elif data[0] == 1:
                if "fresh-music" in existing_channels:
                    ...
                    await db.execute(
                        "UPDATE musicSettings SET musicToggle = ? WHERE guild = ?",
                        (
                            0,
                            ctx.guild.id,
                        ),
                    ...
                    await ctx.send(
                        "<:tickYes:697759553626046546> Deleted fresh music channel and disabled the system."
                    )
                else:
                    await db.execute(
                        "UPDATE musicSettings SET musicToggle = ? WHERE guild = ?",
                    ...
                    await ctx.send(
                        "<:tickYes:697759553626046546> The fresh music channel has already been deleted. But I disabled the system."
                    )
            else:
                if "fresh-music" in existing_channels:
                    channelid = [
                        e.id for e in ctx.guild.channels if e.name == "fresh-music"
                    ][0]
                    msgid = await self.send_player_msg(channelid)
                    await db.execute(
                        "UPDATE musicSettings SET musicMessage = ?, musicToggle = ?, musicChannel = ? WHERE guild = ?",
                        (
                            msgid,
                            1,
                            channelid,
    ...
    async def play(self, ctx, *, query: str):
        ...
        if results["loadType"] == "LOAD_FAILED":
            ...
            return await ctx.send(
                "Oh no, something failed. Please try again.", delete_after=5
            )
    ...
    async def previous(self, ctx):
        ...
        if player.fetch("prev_song") is None:
            return await ctx.send(
                "<:tickNo:697759586538749982> No previous track.", delete_after=5
            )
    ...
    async def search(self, ctx, query):
        ...
        if not results or not results["tracks"]:
            return await ctx.send(
                "<:tickNo:697759586538749982> Nothing found!", delete_after=5
            )
        number = 0
        e = discord.Embed()
                ...
                if content.isdigit():
                    if int(content) > number:
                        ...
                        return await ctx.invoke(
                            ...
                            query=results["tracks"][int(content) - 1]["info"]["uri"],
                        )
    ...
    async def disconnect(self, ctx):
        ...
        if player.is_connected:
            ...
            return await ctx.send(
                "<:tickNo:697759586538749982> Not connected in this server.",
                delete_after=5,
            )
    ...
    async def jump(self, ctx, number: int):
        ...
        if not player.queue:
            await ctx.send(
                "<:tickNo:697759586538749982> Nothing queued.", delete_after=5
            )
        ...
        player.queue.insert(
            0, self.bot.lavalink.player_manager.get(ctx.guild.id).queue[number - 1]
        )
        player.queue.pop(number)
        return await ctx.send(
            "<:tickYes:697759553626046546> Moved to the top of the queue.",
            delete_after=10,
        )
    ...
    async def clear(self, ctx):
        ...
        if not player.queue:
            await ctx.send(
                "<:tickNo:697759586538749982> Nothing queued, use `play <song_name>`.",
                delete_after=5,
            )
            return
        else:
            player.queue.clear()
            return await ctx.send(
                "<:tickYes:697759553626046546> Cleared queue.", delete_after=10
            )
    ...
    async def remove(self, ctx, index: int):
        ...
        return await ctx.send(
            "<:tickYes:697759553626046546> Removed from the queue.", delete_after=10
        )
    ...
    async def now(self, ctx):
        ...
        if player.is_playing:
            ...
            if player.is_connected:
                return await ctx.send(
                    "<:tickNo:697759586538749982> Nothing is playing.", delete_after=5
                )
    ...
    async def skip(self, ctx, number: int = None):
        ...
        if player.current is None:
            return await ctx.send(
                "<:tickNo:697759586538749982> Nothing playing, use `play <song_name>`.",
                delete_after=5,
            )
        if number is not None:
            ...
            player.queue.insert(
                0,
                self.bot.lavalink.player_manager.get(ctx.guild.id).queue[
                    len(player.queue) - 1
                ],
            )
            player.queue.pop(number)
    ...
    async def queue(self, ctx, page: int = 1):
        ...
        if not player.queue:
            return await ctx.send(
                "<:tickNo:697759586538749982> Nothing playing.", delete_after=4
            )
        if player.current.stream:
            dur = "LIVE"
        else:
        ...
        e.add_field(
            name="Up Next:",
            value=f"{await self.draw_queue(ctx.guild.id, page)}",
            inline=False,
        )
        e.set_thumbnail(
            url=f"https://img.youtube.com/vi/{player.current.identifier}/hqdefault.jpg"
        ...
        if len(player.queue) > 10:
            e.set_footer(
                text=f"Page {page}/{math.ceil(len(player.queue) / 10)} | {len(player.queue)} tracks"
            )
            return await ctx.send(
                embed=e, view=queue_msg_buttons(self.bot, ctx.guild.id, 1)
            )
        else:
    ...
    async def draw_queue(self, guild_id, page: int = 1):
        ...
        queue_list = ""
        for i, track in enumerate(
            player.queue[(page - 1) * 10 : (page - 1) * 10 + 10], start=(page - 1) * 10
        ):
            queue_list += "{}) {}\n".format(i + 1, track.title)
        return queue_list
    ...
    async def queue_spotify(self, ctx, player, query):
        ...
        if "track" in parts:
            ...
            if results:
                ...
                await ctx.send(
                    "<:tickNo:697759586538749982> Nothing was found!", delete_after=5
                )
        elif "album" in parts:
            loadingmsg = await send(
                "<a:loading:697759686509985814> Loading spotify info..."
            )
            query = parts[-1]
            results = await self.make_spotify_req(
            ...
            if results:
                ...
                for i in spotify_info:
                    results = await player.node.get_tracks(
                        "ytsearch:{} {}".format(i["name"], i["artists"][0]["name"])
                    )
                    track = AudioTrack(
                        results["tracks"][0], ctx.author.id, recommended=True
                    )
                ...
                await send(
                    f"<:tickYes:697759553626046546> Loaded Album **{albumName}** by **{artistName}!**",
                    delete_after=5,
                )
            else:
                await loadingmsg.delete()
            ...
            if results:
                ...
                await loadingmsg.delete()
                await send(
                    f"<:tickYes:697759553626046546> Loaded Playlist **{playlistName}**!",
                    delete_after=5,
                )
            else:
                await loadingmsg.delete()
    ...
    async def make_spotify_req(self, url):
        if (
            self.spotify_token
            and not self.spotify_token["expires_at"] - int(time.time()) < 60
        ):
            token = self.spotify_token["access_token"]
        else:
            auth_header = base64.b64encode(
                (self.bot.spotify_id + ":" + self.bot.spotify_secret).encode("ascii")
            )
            headers = {"Authorization": "Basic %s" % auth_header.decode("ascii")}
            slink = "https://accounts.spotify.com/api/token"
            async with self.bot.session.post(
                slink, data={"grant_type": "client_credentials"}, headers=headers
            ) as r:
                if r.status != 200:
                    print(
                        color(
            ...
            print(
                color(
                    "Created a new access token for Spotify!",
                    fore=self.bot.colors["green"],
                )
            )
        async with self.bot.session.request(
            "GET", url, headers={"Authorization": "Bearer {0}".format(token)}
        ) as r:
            if r.status != 200:
                print(
                    color(
