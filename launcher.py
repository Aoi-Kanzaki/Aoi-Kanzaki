...
if not os.path.exists("config.json"):
    with open("config.json", "w") as f:
        parse = json.loads(
            '{"prefix": "None", "guildid": "None", "token": "None", "spotify_id": "None", "spotify_secret": "None", "mongoURI": "None"}'
        )
...
def main_menu():
    while True:
        ...
        print(
            color("1.", fore=colors["cyan"])
            + color(" Run Fresh", fore=colors["purple"])
        )
        print(
            color("2.", fore=colors["cyan"])
            + color(" Setup Config", fore=colors["purple"])
        )
        print(
            color("3.", fore=colors["cyan"])
            + color(" Install Requirements", fore=colors["purple"])
        )
...
def setup_menu():
    ...
    while True:
        ...
        print(
            color("1.", fore=colors["cyan"])
            + color(" Setup Token", fore=colors["purple"])
        )
        print(
            color("2.", fore=colors["cyan"])
            + color(" Setup Guild ID", fore=colors["purple"])
        )
        print(
            color("3.", fore=colors["cyan"])
            + color(" Setup Prefix", fore=colors["purple"])
        )
        print(
            color("4.", fore=colors["cyan"])
            + color(" Setup Spotify Client ID", fore=colors["purple"])
        )
        print(
            color("5.", fore=colors["cyan"])
            + color(" Setup Spotify Client Secret", fore=colors["purple"])
        )
        print(
            color("6.", fore=colors["cyan"])
            + color(" Setup Mongo URI", fore=colors["purple"])
        )
...
def change_config(key, value):
    if value is "":
        return input(
            color(
                "\nYou cannot set the config option as nothing. Press enter to continue.",
                fore=colors["red"],
            )
        )
    else:
